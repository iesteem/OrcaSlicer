// 树状支撑由Thomas Rahm开发，大致基于CuraEngine的树状支撑。
// Thomas Rahm树状支撑的原始来源：
// https://github.com/ThomasRahm/CuraEngine
//
// 原始CuraEngine版权：
// 版权所有 (c) 2021 Ultimaker B.V.
// CuraEngine根据AGPLv3或更高版本发布。
...
//FIXME Machine border is currently ignored. 机器边界当前被忽略。
static Polygons calculateMachineBorderCollision(Polygon machine_border)
{
    // Put a border of 1m around the print volume so that we don't collide. 在打印体积周围放置1米的边界，以避免碰撞。
#if 1
...
void TreeModelVolumes::precalculate(const PrintObject& print_object, const coord_t max_layer, std::function<void()> throw_on_cancel)
{
    ...
    // Get the config corresponding to one mesh that is in the current group. Which one has to be irrelevant. 获取与当前组中一个网格对应的配置。选择哪个网格无关紧要。
    // Not the prettiest way to do this, but it ensures some calculations that may be a bit more complex 这不是最优雅的方式，但确保了一些可能更复杂的计算
...

    {
        ...
        // there is no need to calculate them. So just skip these. 理论上可能在尖端发生，半径在两层之间变化太大，以至于跳过一个取整步骤（即存在半径r使得ceilRadius(radius(dtt))<ceilRadius(r)<ceilRadius(radius(dtt+1)))。
        // 这样的半径在树中不会合理出现，很可能不会被请求，因此无需计算它们。直接跳过这些。
    ...
    // Overhead with an assumed worst case of 6600 layers was about 2ms 假设最坏情况下6600层的开销约为2毫秒
    ...
    // Copy to deque to use in parallel for later. 复制到deque以供后续并行使用。
    std::vector<RadiusLayerPair> relevant_avoidance_radiis{ radius_until_layer.begin(), radius_until_layer.end() };

    // Append additional radiis needed for collision. 追加碰撞所需的其他半径。
    // To calculate collision holefree for every radius, the collision of radius m_increase_until_radius will be required. 要计算每个半径的无孔碰撞，将需要m_increase_until_radius半径的碰撞。
    radius_until_layer[ceilRadius(m_increase_until_radius + m_current_min_xy_dist_delta)] = max_layer;
    // Collision for radius 0 needs to be calculated everywhere, as it will be used to ensure valid xy_distance in drawAreas. 半径0的碰撞需要在所有地方计算，因为它将用于确保drawAreas中有效的xy_distance。
    radius_until_layer[0] = max_layer;
    if (m_current_min_xy_dist_delta != 0)
        radius_until_layer[m_current_min_xy_dist_delta] = max_layer;

    // Now that required_avoidance_limit contains the maximum of ild and regular required radius just copy. 既然required_avoidance_limit包含了ild和常规所需半径的最大值，直接复制。
    std::vector<RadiusLayerPair> relevant_collision_radiis{ radius_until_layer.begin(), radius_until_layer.end() };

    // Calculate the relevant collisions 计算相关的碰撞
    ...
    // Calculate collisions without holes, built from regular collision 计算无孔碰撞，从常规碰撞构建
    calculateCollisionHolefree(relevant_hole_collision_radiis, throw_on_cancel);
    // Let placables be calculated from calculateAvoidance() for better parallelization. 让可放置区域通过calculateAvoidance()计算，以获得更好的并行化。
    ...
    // Calculate the relevant avoidances in parallel as far as possible 尽可能并行计算相关的避让区域
    {
...
// Private. Only called internally by calculateAvoidance() and calculateAvoidanceToModel(), radius is already snapped to grid. 私有方法。仅由calculateAvoidance()和calculateAvoidanceToModel()内部调用，半径已对齐到网格。
...
const Polygons& TreeModelVolumes::getAvoidance(const coord_t orig_radius, LayerIndex layer_idx, AvoidanceType type, bool to_model, bool min_xy_dist) const
{
    ...
    // Retrive failed and correct result was calculated. Now it has to be retrived. 检索失败，但已计算出正确结果。现在必须重新检索。
...
const Polygons& TreeModelVolumes::getPlaceableAreas(const coord_t orig_radius, LayerIndex layer_idx, std::function<void()> throw_on_cancel) const
{
    ...
    if (orig_radius == 0)
        // Placable areas for radius 0 are calculated in the general collision code. 半径为0的可放置区域在通用碰撞代码中计算。
...
const Polygons& TreeModelVolumes::getWallRestriction(const coord_t orig_radius, LayerIndex layer_idx, bool min_xy_dist) const
{
    assert(layer_idx > 0);
    if (layer_idx == 0) 
        // Should never be requested as there will be no going below layer 0 ..., 应该永远不会被请求，因为没有低于第0层的可能...
...
// Calculate collisions and placable areas for radius and for layer 0 to max_layer_idx inclusive. 计算指定半径从第0层到max_layer_idx（含）的碰撞和可放置区域。
...
//    assert(radius == this->ceilRadius(radius));
    ...
    // Process the outlines from least layers to most layers so that the final union will run over the longest vector. 从最少层到最多层处理轮廓，以便最终的合并操作处理最长的向量。
    ...
    // Layer range for which the collisions will be calculated. 将计算碰撞的层范围。
    ...
    for (size_t outline_idx : layer_outline_indices)
        if (const std::vector<Polygons> &outlines = m_layer_outlines[outline_idx].second; ! outlines.empty()) {
                    ...
                    for (LayerIndex layer_idx = range.begin(); layer_idx != range.end(); ++ layer_idx) {
                        ...
                        for (int i = 1; i <= z_distance_top_layers; ++ i)
                            if (int j = layer_idx + i; j < int(outlines.size())) {
                                ...
                                //    +-----+
                                //   o +-----+
                                // If just the collision above is accumulated the overhang will get overwritten by the xy_distance of the 如果只累积上方的碰撞，悬垂区域将被悬垂下方层的XY距离覆盖 如果只累积上方的碰撞，悬垂区域将被悬垂下方层的XY距离覆盖
                                //
                                // This only causes issues if the overhang area is thinner than xy_distance 只有在悬垂区域比XY距离更薄时才会引起问题
                                // Just accumulating areas of the model above without the xy distance is also problematic, as then support 只累积上方模型区域而不考虑XY距离也有问题，因为支撑
                                // may get closer to the model (on the diagonal downwards) than the user intended. Example (s = support): 可能会比用户预期的更靠近模型（沿对角线向下）。示例（s = 支撑）：
                                //  +-----+
                                //   +-----+
                                //   +-----+
                                //   s+-----+
            ...
            if (calculate_placable) {
                // Now calculate the placable areas. 现在计算可放置区域。
                ...
                (const tbb::blocked_range<LayerIndex>& range) {
                    for (LayerIndex layer_idx = range.begin(); layer_idx != range.end(); ++ layer_idx) {
                        ...
                        Polygons placable = diff(
                            // Inflate the surface to sit on by the separation distance to increase chance of a support being placed on a sloped surface. 将待放置表面按分离距离膨胀，以增加支撑被放置在倾斜表面上的机会。
                ...
                // Calculating just the collision areas. 仅计算碰撞区域。
            }
        }
#ifdef SLIC3R_TREESUPPORTS_PROGRESS
    {
        ...
        for (LayerIndex layer_idx = range.begin(); layer_idx < range.end(); ++ layer_idx) {
            for (RadiusLayerPair key : keys)
                if (layer_idx <= key.second) {
                    // Logically increase the collision by m_increase_until_radius 逻辑上按m_increase_until_radius增加碰撞
...
{
    // For every RadiusLayer pair there are 3 avoidances that have to be calculated. 每个RadiusLayer对有3种避让需要计算。
    // Prepare tasks for parallelization. 准备并行化任务。
    ...
    for (int iter_idx = 0; iter_idx < 2 * int(keys.size()) * int(AvoidanceType::Count); ++ iter_idx) {
        ...
        // Ensure start_layer is at least 1 as if no avoidance was calculated yet getMaxCalculatedLayer() returns -1. 确保start_layer至少为1，因为如果尚未计算避让，getMaxCalculatedLayer()返回-1。
    ...
    tbb::parallel_for(tbb::blocked_range<size_t>(0, avoidance_tasks.size(), 1),
        ...
        for (size_t task_idx = range.begin(); task_idx < range.end(); ++ task_idx) {
            ...
            // The following loop propagating avoidance regions bottom up is inherently serial. 以下自底向上传播避让区域的循环本质上是串行的。
            ...
            // Limiting the offset step so that unioning the shrunk latest_avoidance with the current layer collisions 限制偏移步长，以便将收缩后的latest_avoidance与当前层碰撞合并
            ...
            if (last_move_step < scaled<float>(0.05)) {
                assert(move_steps > 1);
                if (move_steps > 1) {
                    // Avoid taking a very short last step, stretch the other steps a bit instead. 避免最后一步非常短，而是稍微拉伸其他步骤。
            ...
            for (LayerIndex layer_idx = task.start_layer; layer_idx <= task.max_required_layer; ++ layer_idx) {
                // Merge current layer collisions with shrunk last_avoidance. 合并当前层碰撞与收缩后的last_avoidance。
                ...
                // For mildly steep branch angles only one step will be taken. 对于较缓的分支角度，只需一步。
        ...
        [this, &data, radius, start_layer, &throw_on_cancel](const tbb::blocked_range<LayerIndex>& range) {
            for (LayerIndex layer_idx = range.begin(); layer_idx < range.end(); ++ layer_idx) {
                data[layer_idx - start_layer] = offset(
                    ...
                    // As a placeable area is calculated by (collision of the layer below) - (collision of the current layer) and the collision is offset by xy_distance, 由于可放置区域是通过（下方层的碰撞）-（当前层的碰撞）计算的，并且碰撞按xy_distance偏移，
...
{
    // Wall restrictions are mainly important when they represent actual walls that are printed, and not "just" the configured z_distance, because technically valid placement is no excuse for moving through a wall. 壁限制在表示实际打印的壁时尤为重要，而不仅仅是配置的z_distance，因为技术上有效的放置并不是穿过壁的借口。
    // As they exist to prevent accidentially moving though a wall at high speed between layers like thie (x = wall,i = influence area,o= empty space,d = blocked area because of z distance) Assume maximum movement distance is two characters and maximum safe movement distance of one character 它们的存在是为了防止意外地高速穿过壁（x = 壁，i = 影响区域，o = 空白区域，d = 由于z距离而阻塞的区域）。假设最大移动距离为两个字符，最大安全移动距离为一个字符
    ...
    // The radius for the upper collission has to be 0 as otherwise one may not enter areas that may be forbidden on layer_idx but not one below (c = not an influence area even though it should ): 上层碰撞的半径必须为0，否则可能无法进入在layer_idx上被禁止但在下方层不被禁止的区域：
    /*
     *  layer z+1:xxxxxiiiiiioo
     *  layer z+0:dddddiiiiiiio
     *  layer z-1:dddocdddddddd
     */
    // Also there can not just the collision of the lower layer be used because if it were: 也不能仅仅使用下层碰撞，因为如果那样：
    /*
     *  layer z+1:dddddiiiiiiiiiio
     *  layer z+0:xxxxxddddddddddc
     *  layer z-1:dddddxxxxxxxxxxc
     */
    // Or of the upper layer be used because if it were: 也不能使用上层碰撞，因为如果那样：
    /*
     *  layer z+1:dddddiiiiiiiiiio
    ...
    // And just offseting with maximum movement distance (and not in multiple steps) could cause: 仅用最大移动距离偏移（而不是多步偏移）可能导致：
    /*
